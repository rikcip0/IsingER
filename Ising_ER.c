...
{ // restituisce numeri secondo distribuzione gauss di media nulla e varianza sigma^2
...
void iteration(int t_MAX, FILE *out)
{
...
int main(int argc, char *argv[])
{
  ...
  sprintf(mcStoriesFile, "./SimulationsResults/out.txt");
  ...
  if (argc == 4)
  { // se non ho messo seed tra gli argomenti lo pesco da un file di numeri random
    FILE *devran = fopen("/dev/urandom", "r");
    fread(&myrand, 4, 1, devran);
    fclose(devran);
  }
  else
    myrand = atoi(argv[4]); // altrimenti uso seed passatomi in input
